...
Exit codes & warnings:
  ...
  cap-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  cap-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  cap-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  cap-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  cap-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/cap-pp-cli/data.db)")
