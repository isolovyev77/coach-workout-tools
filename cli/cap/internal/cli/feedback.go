...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CAP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CAP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cap-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.cap-pp-cli/feedback.jsonl.
When ` + "`CAP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CAP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cap-pp-cli feedback list
  cap-pp-cli feedback list --limit 5
  cap-pp-cli feedback list --json`,
