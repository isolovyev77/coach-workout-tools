...
type Config struct {
	...
	Path           string `toml:"-"`
	TrendaSession string `toml:"session"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://app.trenda.coach",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("TRENDA_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "trenda-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("TRENDA_SESSION"); v != "" {
		cfg.TrendaSession = v
		cfg.AuthSource = "env:TRENDA_SESSION"
	}
	...
	if cfg.AuthSource == "" && cfg.TrendaSession != "" {
		cfg.AuthSource = "config"
	}
