...
Exit codes & warnings:
  ...
  trenda-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  trenda-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  trenda-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  trenda-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  trenda-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/trenda-pp-cli/data.db)")
