...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".trenda-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TRENDA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TRENDA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "trenda-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.trenda-pp-cli/feedback.jsonl.
When ` + "`TRENDA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TRENDA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  trenda-pp-cli feedback list
  trenda-pp-cli feedback list --limit 5
  trenda-pp-cli feedback list --json`,
