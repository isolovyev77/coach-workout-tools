...
Exit codes & warnings:
  ...
  btwb-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  btwb-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  btwb-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  btwb-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  btwb-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "whiteboard":
		return db.UpsertWhiteboard(data)
	case "workout_sessions":
		return db.UpsertWorkoutSessions(data)
	case "tasks":
		return db.UpsertTasks(data)
	case "webwidgets":
		return db.UpsertWebwidgets(data)
	default:
