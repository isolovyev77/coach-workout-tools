...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BTWB_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BTWB_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "btwb-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.btwb-pp-cli/feedback.jsonl.
When ` + "`BTWB_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BTWB_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  btwb-pp-cli feedback list
  btwb-pp-cli feedback list --limit 5
  btwb-pp-cli feedback list --json`,
