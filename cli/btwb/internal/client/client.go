...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "btwb-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	// The Web Widgets API wants a JavaScript-shaped date (see btwb.go). Rewrite
	// it before the dry-run branch so --dry-run previews what would really go.
	params = normalizeWidgetParams(targetURL, params)
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// btwb takes the session as a Cookie on btwb.com and a bearer key on
		// the Web Widgets host; see internal/client/btwb.go.
		if err := applyBtwbAuth(req, c.Config, targetURL); err != nil {
			return nil, 0, err
		}
		if c.Config != nil {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "btwb-pp-cli/1.0.0")
		}
		...
		if resp.StatusCode < 400 {
			converted, convErr := transformBtwbResponse(targetURL, params, respBody)
			if convErr != nil {
				return nil, resp.StatusCode, convErr
			}
			respBody = converted
		}
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if authHeader != "" {
		fmt.Fprintf(os.Stderr, "  %s: %s\n", "_btwb_session_id", maskToken(authHeader))
	}
