...
type Config struct {
	...
	// gym's Web Widgets API on a different host. MemberID is cached at login
	// so commands need not be told whose whiteboard to read.
	WidgetKey string `toml:"widget_key,omitempty"`
	MemberID  int    `toml:"member_id,omitempty"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://btwb.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("BTWB_CONFIG")
	}
	if path == "" {
	...
	if v := os.Getenv("BTWB_SESSION_COOKIE"); v != "" {
		cfg.BtwbSessionCookie = v
		cfg.AuthSource = "env:BTWB_SESSION_COOKIE"
	}
	...
	if cfg.AuthSource == "" && cfg.BtwbSessionCookie != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("BTWB_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
